...
export async function buildSystemPrompt(clientId: string): Promise<string> {
  ...
  const [
    ...
    db
      ...
      .select(
        "name, nutrition_protocol_days(calories, protein_g, fat_g, carbs_g, hydration_ml)",
      )
    ...
    db
      ...
      .select(
        "total_calories, total_protein_g, total_fat_g, total_carbs_g, meal_type, title, logged_at",
      )
      .eq("client_id", clientId)
    ...
    db
      ...
      .select(
        "flow_type, sleep_hours, sleep_quality, energy_level, stress_level, weight_kg, hunger_level, muscle_soreness",
      )
      .eq("client_id", clientId)
      .eq("date", today),
    ...
    db
      .from("programs")
      .select(
        "name, weeks, frequency, program_sessions(id, name, day_of_week, days_of_week)",
      )
      .eq("client_id", clientId)
      .eq("status", "active")
      .order("created_at", { ascending: false })
  ...
  // ── Coach identity ─────────────────────────────────────────────────────────
  const coachProfile =
    coachProfileResult.status === "fulfilled"
      ? (coachProfileResult.value as any)?.data
      : null;
  ...
  // ── Macros targets ────────────────────────────────────────────────────────
  const protocol =
    nutritionProtocol.status === "fulfilled"
      ? nutritionProtocol.value.data
      : null;
  const protocolDay = selectProtocolDayForDate(protocol as any, today);
  ...
  const legacyMealsData =
    legacyMealsResult.status === "fulfilled"
      ? (legacyMealsResult.value.data ?? [])
      : [];

  const totalKcal =
    composerMeals.reduce(
      (s, m) => s + Number((m as any).total_calories ?? 0),
      0,
    ) +
    legacyMealsData.reduce(
      (s: number, m: any) => s + Number(m.estimated_macros?.calories_kcal ?? 0),
      0,
    );

  const totalProtein =
    composerMeals.reduce(
      (s, m) => s + Number((m as any).total_protein_g ?? 0),
      0,
    ) +
    legacyMealsData.reduce(
      (s: number, m: any) => s + Number(m.estimated_macros?.protein_g ?? 0),
      0,
    );
  ...
  const totalFat =
    ...
    legacyMealsData.reduce(
      (s: number, m: any) => s + Number(m.estimated_macros?.fat_g ?? 0),
      0,
    );

  const totalCarbs =
    composerMeals.reduce(
      (s, m) => s + Number((m as any).total_carbs_g ?? 0),
      0,
    ) +
    legacyMealsData.reduce(
      (s: number, m: any) => s + Number(m.estimated_macros?.carbs_g ?? 0),
      0,
    );
  ...
  const mealsLines =
    composerMeals.length > 0 || legacyMealsData.length > 0
      ? [
          ...composerMeals.map((m: any) => {
            ...
            const label =
              m.title ?? MEAL_LABELS[m.meal_type as string] ?? "Repas";
  ...
  const activitiesLine =
    activities.length > 0
      ? activities
          .map(
            (a) =>
              `  - ${a.custom_label ?? a.activity_type} ${a.duration_min}min`,
          )
          .join("\n")
      : "  Aucune";
  ...
  // ── Restrictions ──────────────────────────────────────────────────────────
  const restrictions =
    restrictionsResult.status === "fulfilled"
      ? (restrictionsResult.value.data ?? [])
      : [];
  const restrictionsLine =
    restrictions.length > 0
      ? restrictions
          .map((r) => `${r.label ?? r.body_part} (${r.severity})`)
          .join(", ")
      : "aucune";

  // ── Body composition — full history ───────────────────────────────────────
  const bilans =
    bodyCompResult.status === "fulfilled"
      ? (bodyCompResult.value.data ?? [])
      : [];
  ...
  const extractValues = (bilan: any): Record<string, number> => {
    ...
    for (const r of (bilan?.assessment_responses ??
  ...
  const latestBilanData = bilans.length
    ? extractValues(bilans[bilans.length - 1])
    : {};
  ...
  const totalWeightDelta =
    latestWeight != null && firstWeight != null
      ...
      : null;
  const totalBFDelta =
    latestBF != null && firstBF != null
      ? +(latestBF - firstBF).toFixed(1)
      : null;
  ...
  if (latestWeight == null) {
    ...
    if (bilans[0]?.bilan_date && bilans.length > 1) {
      lines.push(
        `Début du suivi (${fmtDate(bilans[0].bilan_date)}): ${firstWeight ?? "?"}kg${firstBF != null ? ` | MG ${firstBF}%` : ""}`,
      );
    }
    // Intermediate bilans
    if (bilans.length > 2) {
      bilans.slice(1, -1).forEach((b: any) => {
        ...
        lines.push(
          `  ${fmtDate(b.bilan_date)}: ${w}kg${bf != null ? ` | MG ${bf}%` : ""}`,
        );
      });
    }
    const lastBilan = bilans[bilans.length - 1];
    lines.push(
      `Actuel (${fmtDate(lastBilan.bilan_date)}): ${latestWeight}kg${latestBF != null ? ` | MG ${latestBF}%` : ""}${latestLBM != null ? ` | MM ${latestLBM}kg` : ""}`,
    );
    if (totalWeightDelta !== null) {
      const sign = totalWeightDelta > 0 ? "+" : "";
      lines.push(
        `PROGRESSION TOTALE: ${sign}${totalWeightDelta}kg poids${totalBFDelta !== null ? ` | ${totalBFDelta > 0 ? "+" : ""}${totalBFDelta}% MG` : ""}`,
      );
    }
  ...
  const trendRows =
    nutritionTrendsResult.status === "fulfilled"
    ...
    trendRows.length > 0
      ? trendRows
          .map((row: any) => {
            ...
            const kcalPct =
              targetKcal > 0 ? Math.round((kcal / targetKcal) * 100) : 0;
            const proteinOk =
              targetProtein > 0 ? protein >= targetProtein : true;
  ...
  const morningLine = morningCheckin
    ? [
        morningCheckin.sleep_hours != null
          ? `sommeil ${morningCheckin.sleep_hours}h`
          : null,
        morningCheckin.sleep_quality != null
          ? `qualité ${QUALITY_LABELS[morningCheckin.sleep_quality] ?? morningCheckin.sleep_quality}`
          : null,
        morningCheckin.energy_level != null
          ? `énergie ${ENERGY_LABELS[morningCheckin.energy_level] ?? morningCheckin.energy_level}/5`
          : null,
        morningCheckin.weight_kg != null
          ? `poids ${morningCheckin.weight_kg}kg`
          : null,
      ]
        .filter(Boolean)
        .join(", ")
  ...
  const eveningLine = eveningCheckin
    ? [
        eveningCheckin.energy_level != null
          ? `énergie ${ENERGY_LABELS[eveningCheckin.energy_level] ?? eveningCheckin.energy_level}/5`
          : null,
        eveningCheckin.stress_level != null
          ? `stress ${STRESS_LABELS[eveningCheckin.stress_level] ?? eveningCheckin.stress_level}`
          : null,
        eveningCheckin.muscle_soreness != null
          ? `courbatures ${eveningCheckin.muscle_soreness}/4`
          : null,
        eveningCheckin.hunger_level != null
          ? `faim ${eveningCheckin.hunger_level}/4`
          : null,
      ]
        .filter(Boolean)
        .join(", ")
