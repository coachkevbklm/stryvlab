...
function dominantMacro(item: FoodItem): AdvisorMacroKey | null {
...
export function evaluateFoodCompatibility({
  ...
  const highFat = food.fat_per_100g >= 12
  const highCarb = food.carbs_per_100g >= 18
  const highProtein = food.protein_per_100g >= 18
