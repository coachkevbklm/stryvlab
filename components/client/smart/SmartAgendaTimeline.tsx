...
import { ForkKnife, Drop, Barbell, PersonSimpleRun, Moon } from '@phosphor-icons/react'
      ...
      {entries.length === 0 ? (
        ...
        <div className="flex flex-col gap-2.5">
          {entries.map(e => {
            ...
            const highlight = e.kind === 'workout'
              ? 'bg-[#ffe01e]/[0.06] border border-[#ffe01e]/20'
              : 'bg-white/[0.02]'

            const Body = (
              <div className={`flex items-center gap-3 p-2.5 rounded-xl ${highlight}`}>
                <div className="w-[44px] text-[10px] text-white/40 font-bold tabular-nums shrink-0">{timeOf(e.start_iso)}</div>
