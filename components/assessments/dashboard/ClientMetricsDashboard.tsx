...
const TRACKED_FIELDS: { key: string; label: string; unit?: string; color?: string }[] = [
  ...
  { key: 'muscle_mass_kg', label: 'Masse musculaire',    unit: 'kg',   color: '#2d9f6f' },
  ...
  { key: 'visceral_fat',   label: 'Graisse viscérale',   unit: '',     color: '#e08c3a' },
  { key: 'bmi',            label: 'IMC',                 unit: '',     color: '#6D6D6D' },
  { key: 'bmr_kcal',       label: 'Métabolisme de base', unit: 'kcal', color: '#d4a017' },
  ...
  { key: 'sleep_hours',    label: 'Heures de sommeil',   unit: 'h',    color: '#7c9dd4' },
...
export default function ClientMetricsDashboard({ clientId, refreshKey = 0 }: Props) {
  ...
  if (loading) {
    return <div className="text-center py-8 text-secondary text-sm">Chargement des métriques…</div>
  }
  ...
  if (!hasData) {
    return (
      <div className="text-center py-12 text-secondary">
        <p className="font-bold mb-1">Aucune donnée disponible</p>
        <p className="text-xs opacity-60">Les métriques apparaîtront après un import CSV ou un bilan complété</p>
      </div>
    )
  }
        ...
        return (
          <div key={f.key} className="bg-surface rounded-card shadow-soft-out p-4">
            ...
            <div className="mt-2 flex items-center justify-between text-xs text-secondary">
              ...
              <span>{data.length} mesure{data.length > 1 ? 's' : ''}</span>
            </div>
          </div>
        )
      })}
