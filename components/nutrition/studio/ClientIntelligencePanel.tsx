...
export default function ClientIntelligencePanel({
  ...
  const [selectedMissingData, setSelectedMissingData] =
  ...
  const handleMissingDataApply = async (
    field: string,
    value: number | string,
  ) => {
    if (!clientId) return;
    ...
    setCompleting(true);
    try {
      // Map field to biometricsConfig key
      let updateData: Record<string, unknown> = {};

      if (field === "bmr_kcal_measured") {
        updateData = {
          bmr_kcal_measured: value,
          bmr_source: "calculated",
        };
        onBiometricsChange({
          bmr_kcal_measured: Number(value),
          bmr_source: "calculated",
        });
      } else if (field === "weight_kg") {
        updateData = { weight_kg: Number(value) };
        onBiometricsChange({ weight_kg: Number(value) });
      } else if (field === "height_cm") {
        updateData = { height_cm: Number(value) };
        onBiometricsChange({ height_cm: Number(value) });
      } else if (field === "body_fat_pct") {
        updateData = { body_fat_pct: Number(value) };
        onBiometricsChange({ body_fat_pct: Number(value) });
      } else if (field === "daily_steps") {
        updateData = { daily_steps: Number(value) };
        onTrainingChange({ dailySteps: Number(value) });
      }
      ...
      if (!res.ok) {
        throw new Error("Erreur persistance");
      }

      setSelectedMissingData(null);
    } catch (err) {
      console.error("Erreur application donnée:", err);
    } finally {
      setCompleting(false);
    }
  };
  ...
  return (
    <>
      <div className="overflow-y-auto scrollbar-hide p-4 space-y-4">
        ...
        {/* Missing data alerts — now clickable */}
        <MissingDataAlerts
      ...
      {/* Complete missing data modal */}
      <CompleteMissingDataModal
        isOpen={selectedMissingData !== null}
