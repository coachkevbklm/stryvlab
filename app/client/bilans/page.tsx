...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${s.className}`}
    >
      {s.icon}
      {s.label}
    </span>
        ...
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-secondary mt-0.5">
              {submissions.length} bilan{submissions.length !== 1 ? "s" : ""}
            </p>
          </div>
          <Image
            src="/images/logo.png"
      ...
      <main className="max-w-lg mx-auto px-6 py-6">
        {submissions.length === 0 ? (
          <div className="bg-surface rounded-card shadow-soft-out p-8 text-center">
            <ClipboardList
              size={36}
              className="text-secondary mx-auto mb-3 opacity-40"
            />
            <p className="text-sm text-secondary">
              Aucun bilan pour le moment.
            </p>
            <p className="text-xs text-secondary/60 mt-1">
              Ton coach t'enverra un lien quand il en crée un.
            </p>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
            {submissions.map((sub: any) => {
              ...
              return (
                <div
                  key={sub.id}
                  className="bg-surface rounded-card shadow-soft-out overflow-hidden"
                >
