...
export async function POST(req: NextRequest) {
  ...
  if (!client_id || !formula_id) {
    return NextResponse.json({ error: 'client_id et formula_id requis' }, { status: 400 })
  }
  ...
  if (fErr || !formula) return NextResponse.json({ error: 'Formule introuvable' }, { status: 404 })
  ...
  if (cErr || !client) return NextResponse.json({ error: 'Client introuvable' }, { status: 404 })
  if (!client.email) return NextResponse.json({ error: 'Ce client n\'a pas d\'email — requis pour Stripe' }, { status: 422 })
