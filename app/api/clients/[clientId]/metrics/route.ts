...
async function getOrCreateManualTemplate(
  db: ReturnType<typeof serviceClient>,
  coachId: string,
): Promise<string> {
...
export async function GET(
  ...
  const { data: submissions, error } = await db
    ...
    .select(
      `id, submitted_at, bilan_date, created_at, assessment_responses(field_key, value_number)`,
    )
  ...
  for (const sub of submissions ?? []) {
    const rawDate: string =
      sub.bilan_date ?? sub.submitted_at ?? sub.created_at;
...
export async function POST(
  ...
  if (Object.keys(values).length === 0) {
    return NextResponse.json(
      { error: "At least one value required" },
      { status: 400 },
    );
  }
  ...
  if (existing) {
    return NextResponse.json(
      { error: "Une mesure existe déjà pour cette date" },
      { status: 409 },
    );
  }
  ...
  const { data: submission, error: subError } = await db
    ...
    .insert({
      ...
      template_snapshot: {
        blocks: [{ id: "csv_import_block", module: "biometrics" }],
      },
      status: "completed",
      filled_by: "coach",
  ...
  if (respError) {
    ...
    return NextResponse.json(
      { error: "Erreur insertion réponses" },
      { status: 500 },
    );
  }
